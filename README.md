...
| Database | SQL Server - dedicated `madauthor` database, with `madauthorhangfire` for Hangfire (creds in `.env`, gitignored) |
...
Decisions resolved 2026-05-20: DB = dedicated `madauthor` plus separate `madauthorhangfire`; storage = local filesystem; auth = JWT (with ASP.NET Identity as the user store); email = SMTP via MailKit; MOBI dropped; cover-gen deferred to Phase 5; outline-approval default true.
