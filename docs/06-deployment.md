...
- Hit `https://yoursite.com/api/health/ready` — should return `{ status: "ready", db: true }`.
...
- **Same caveats as Fly.io** about SQL firewall: your `41.185.13.202` host must accept connections from the IIS host's egress IP.
...
- **Stateful pieces are external.** The SQL Server you already have at `41.185.13.202` continues to be the data plane. The container only has a small persistent volume for uploaded files + rendered exports.
...
Critical: your SQL Server at `41.185.13.202` must accept connections from Fly.io's egress IPs. Two options:
...
- **Allow `0.0.0.0/0` with strong SQL auth** (your current `madproducts` user). Less ideal, but pragmatic for a solo project on a non-PCI workload.
...
docker run --rm -p 8080:8080 `
  -e "ConnectionStrings__DefaultConnection=Server=41.185.13.202;Database=madapi;User Id=madproducts;Password=***REDACTED-DB-PASS***;TrustServerCertificate=True;Encrypt=True;" `
  -e JWT_SIGNING_KEY=<paste your signing key> `
  -e UNSPLASH_ACCESS_KEY=<paste your unsplash key> `
  -e SMTP_HOST=smtp.dreamhost.com `
  -e SMTP_PORT=465 `
  -e SMTP_SECURE=true `
  -e SMTP_USER=admin@madleads.ai `
...
flyctl secrets set `
  ConnectionStrings__DefaultConnection="Server=41.185.13.202;Database=madapi;User Id=madproducts;Password=***REDACTED-DB-PASS***;TrustServerCertificate=True;Encrypt=True;" `
  JWT_SIGNING_KEY="<your signing key>" `
  UNSPLASH_ACCESS_KEY="<your unsplash key>" `
  SMTP_HOST=smtp.dreamhost.com `
  SMTP_PORT=465 `
  SMTP_SECURE=true `
  SMTP_USER=admin@madleads.ai `
  SMTP_PASS="<your smtp pass>" `
  SMTP_FROM_ADDRESS="admin@madleads.ai" `
...
```powershell
flyctl certs add author.yourdomain.com
# Add the CNAME flyctl prints to your DNS.
# Wait a minute, then:
flyctl certs check author.yourdomain.com
```
...
- SQL Server is whatever your current `41.185.13.202` host costs — nothing changes.
