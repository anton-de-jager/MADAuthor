...
import { NotificationsService, AiTaskRealtimeEvent } from '../../../core/signalr/notifications.service';
import { ToastService } from '../../../core/ui/toast.service';

// ----------------------------------------------------------------------------
// DTO shapes -- mirror MadAuthor.Contracts.AiTasks.AiTaskDtos.cs.
// ASP.NET serialises the AiTaskStatus byte enum as a number (0-6) since no
...
interface AiTaskDetail {
  ...
  status: number | StatusName;
  priority: number;
  attachments: AiTaskAttachment[];
  createdDate: string;
  updatedDate: string | null;
}

interface AiTaskListResponse {
  active: AiTaskSummary[];
  toBeDeployed: AiTaskSummary[];
  terminal: AiTaskSummary[];
}
...
interface UiTask extends Omit<AiTaskSummary, 'status'> {
  status: StatusName;
}
    ...
    <div class="p-8 max-w-7xl mx-auto">
      ...
      <div class="flex items-start justify-between mb-6">
        ...
        <div class="flex items-center gap-2">
          <button type="button" (click)="openTemplates()" title="Templates" aria-label="Templates"
          ...
          <button type="button" (click)="openImport()" title="Import" aria-label="Import"
            class="w-9 h-9 inline-flex items-center justify-center text-ink-300 hover:text-ink-100 border border-ink-700/60 hover:border-ink-500 rounded-md transition">
            &darr;
          </button>
          <button type="button" (click)="openNew()" title="New task" aria-label="New task"
            class="w-9 h-9 inline-flex items-center justify-center text-ink-300 hover:text-ink-100 border border-ink-700/60 hover:border-ink-500 rounded-md transition">
            +
          </button>
          <button type="button" (click)="refresh()" title="Refresh" aria-label="Refresh"
            class="w-9 h-9 inline-flex items-center justify-center text-ink-300 hover:text-ink-100 border border-ink-700/60 hover:border-ink-500 rounded-md transition">
            &#x21bb;
          </button>
        </div>
      ...
      <div class="flex flex-wrap items-center gap-3 mb-3">
        ...
        <select [value]="statusFilter()" (change)="setStatusFilter(asSelect($event))" aria-label="Filter by status"
        ...
        <select [value]="terminalLimit()" (change)="setTerminalLimit(asSelect($event))" aria-label="Show last completed rows"
          class="bg-ink-900/60 border border-ink-700 rounded-md text-sm px-3 py-1.5 text-ink-100 focus:border-brand-500 focus:outline-none">
          ...
          <div (click)="openEdit(t)"
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex items-center gap-2 flex-shrink-0 self-start">
                ...
                <button type="button" (click)="askDelete(t, $event)" title="Delete task" aria-label="Delete task"
      ...
      <div class="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4"
        ...
        <div class="glass rounded-xl p-6 max-w-xl w-full" (click)="stop($event)">
          <div class="flex items-start justify-between mb-4">
            ...
            <button type="button" (click)="closeEdit()" title="Close" aria-label="Close"
          ...
          <form [formGroup]="taskForm" (ngSubmit)="saveTask()" class="space-y-3">
                ...
                <div class="space-y-1 mt-1">
                  @for (a of editingAttachments(); track a.filename) {
                    <div class="flex items-center justify-between text-sm bg-ink-900/40 border border-ink-700/60 rounded-md px-3 py-1.5">
                      ...
                      <button type="button" (click)="removeAttachment(a)" title="Remove attachment" aria-label="Remove attachment"
            ...
            <div class="flex items-center justify-end gap-2 pt-2">
              <button type="button" (click)="closeEdit()" title="Cancel" aria-label="Cancel"
                class="w-9 h-9 inline-flex items-center justify-center text-ink-300 hover:text-ink-100 border border-ink-700/60 hover:border-ink-500 rounded-md">
                &times;
              </button>
              <button type="submit" [disabled]="taskForm.invalid || saving()" title="Save" aria-label="Save"
      ...
      <div class="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4"
        ...
        <div class="glass rounded-xl p-6 max-w-2xl w-full" (click)="stop($event)">
          <div class="flex items-start justify-between mb-4">
            ...
            <button type="button" (click)="closeImport()" title="Close" aria-label="Close"
              class="w-9 h-9 inline-flex items-center justify-center text-ink-300 hover:text-ink-100 border border-ink-700/60 hover:border-ink-500 rounded-md">
              &times;
            </button>
          </div>
          ...
          <div class="flex items-center justify-end gap-2 pt-4">
            <button type="button" (click)="loadStaticFile()" title="Load /outstanding-tasks.json" aria-label="Load outstanding tasks"
              class="w-9 h-9 inline-flex items-center justify-center text-ink-300 hover:text-ink-100 border border-ink-700/60 hover:border-ink-500 rounded-md">
              &darr;
            </button>
            <button type="button" (click)="closeImport()" title="Cancel" aria-label="Cancel"
              class="w-9 h-9 inline-flex items-center justify-center text-ink-300 hover:text-ink-100 border border-ink-700/60 hover:border-ink-500 rounded-md">
              &times;
            </button>
            <button type="button" (click)="runImport()" [disabled]="!importJson().trim() || saving()" title="Import" aria-label="Import"
              class="w-9 h-9 inline-flex items-center justify-center text-emerald-300 hover:text-emerald-200 border border-emerald-500/40 hover:border-emerald-400 rounded-md disabled:opacity-40 disabled:cursor-not-allowed">
              &#10003;
            </button>
          </div>
      ...
      <div class="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4"
        ...
        <div class="glass rounded-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto" (click)="stop($event)">
          <div class="flex items-start justify-between mb-4">
            ...
            <button type="button" (click)="closeTemplates()" title="Close" aria-label="Close"
              class="w-9 h-9 inline-flex items-center justify-center text-ink-300 hover:text-ink-100 border border-ink-700/60 hover:border-ink-500 rounded-md">
              &times;
            </button>
          </div>
              ...
              <div class="bg-ink-900/40 border border-ink-700/60 rounded-md px-3 py-2">
                <div class="flex items-center justify-between gap-2">
                  ...
                  <div class="flex items-center gap-2 flex-shrink-0">
                    <button type="button" (click)="useTemplate(tpl)" title="Use this template" aria-label="Use this template"
                    ...
                    <button type="button" (click)="deleteTemplate(tpl)" title="Delete template" aria-label="Delete template"
          ...
          <form [formGroup]="templateForm" (ngSubmit)="createTemplate()" class="space-y-2 border-t border-ink-700/60 pt-4">
            ...
            <div class="flex items-center justify-end">
              <button type="submit" [disabled]="templateForm.invalid || saving()" title="Create template" aria-label="Create template"
                class="w-9 h-9 inline-flex items-center justify-center text-emerald-300 hover:text-emerald-200 border border-emerald-500/40 hover:border-emerald-400 rounded-md disabled:opacity-40 disabled:cursor-not-allowed">
                +
              </button>
            </div>
      ...
      <div class="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4"
        ...
        <div class="glass rounded-xl p-6 max-w-md w-full" (click)="stop($event)">
          ...
          <div class="flex items-center justify-end gap-2">
            <button type="button" (click)="cancelDelete()" title="Cancel" aria-label="Cancel"
              class="w-9 h-9 inline-flex items-center justify-center text-ink-300 hover:text-ink-100 border border-ink-700/60 hover:border-ink-500 rounded-md">
              &times;
            </button>
            <button type="button" (click)="confirmDelete()" title="Delete" aria-label="Confirm delete"
...
export class AiPageComponent implements OnInit, OnDestroy {
  ...
  editing = signal<AiTaskDetail | null>(null);
  editingAttachments = signal<AiTaskAttachment[]>([]);
  ...
  templates = signal<AiPromptTemplate[]>([]);
  ...
  async ngOnInit() {
    ...
    try {
      await this.notifications.joinAiTasks();
      this.joinedHub = true;
    } catch {
      this.toasts.push('Realtime updates unavailable. Use Refresh to reload.', 8000);
    }

    this.sub = this.notifications.aiTaskEvents$.subscribe((ev) => this.handleRealtime(ev));
  }

  ngOnDestroy() {
    this.sub?.unsubscribe();
    if (this.joinedHub) {
      this.notifications.leaveAiTasks().catch(() => void 0);
    }
  }
  ...
  refresh() {
    ...
    this.http
      .get<AiTaskListResponse>('/api/ai-tasks', { params: { limit } })
      .subscribe({
        next: (res) => {
          const flat: UiTask[] = [
  ...
  // ---- Helpers -----------------------------------------------------------
  private toUi(t: AiTaskSummary | AiTaskDetail): UiTask {
    return {
      id: t.id,
      title: t.title,
  ...
  openEdit(t: UiTask) {
    ...
    this.http.get<AiTaskDetail>(`/api/ai-tasks/${t.id}`).subscribe({
      next: (detail) => {
  ...
  private uploadFiles(taskId: number, files: File[]) {
    ...
    this.http.post<AiTaskDetail>(`/api/ai-tasks/${taskId}/attachments`, fd).subscribe({
      next: (detail) => {
        this.editingAttachments.set(detail.attachments ?? []);
  ...
  removeAttachment(a: AiTaskAttachment) {
    ...
    this.http.delete(`/api/ai-tasks/${id}/attachments/${encoded}`).subscribe({
      next: () => {
  ...
  saveTask() {
    ...
    if (current?.id) {
      ...
      const url = `/api/ai-tasks/${current.id}` + (movingFromTerminal ? '?override=true' : '');
      this.http.patch<AiTaskDetail>(url, payload).subscribe({
        next: () => {
      ...
      this.http.post<AiTaskDetail>('/api/ai-tasks', payload).subscribe({
        next: (created) => {
          const files = this.pendingFiles();
          if (files.length > 0) {
            const fd = new FormData();
            for (const f of files) fd.append('files', f);
            this.http.post(`/api/ai-tasks/${created.id}/attachments`, fd).subscribe({
              next: () => {
                this.saving.set(false);
                this.closeEdit();
              },
  ...
  confirmDelete() {
    ...
    if (!t) return;
    this.http.delete(`/api/ai-tasks/${t.id}`).subscribe({
      next: () => {
        this.pendingDelete.set(null);
  ...
  runImport() {
    ...
    this.saving.set(true);
    this.http
      .post<ImportBulkResponse>('/api/ai-tasks/import-bulk', { items })
      .subscribe({
        next: (res) => {
          this.saving.set(false);
  ...
  private loadTemplates() {
    this.http.get<AiPromptTemplate[]>('/api/ai-prompt-templates').subscribe({
  ...
  createTemplate() {
    ...
    this.saving.set(true);
    this.http.post<AiPromptTemplate>('/api/ai-prompt-templates', v).subscribe({
      next: (tpl) => {
        this.saving.set(false);
  ...
  deleteTemplate(tpl: AiPromptTemplate) {
    this.http.delete(`/api/ai-prompt-templates/${tpl.id}`).subscribe({
      next: () => this.templates.update((list) => list.filter((t) => t.id !== tpl.id)),
      error: () => this.toasts.push('Failed to delete template.'),
    });
  }
  useTemplate(tpl: AiPromptTemplate) {
    this.closeTemplates();
    this.openNew();
