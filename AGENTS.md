...
Full architecture lives in `docs/01-architecture.md` … `docs/08-Codex-task-system.md`.
...
| `workers/Codex-desktop/`           | Codex Desktop integration            |
| `.Codex/scanner/` + `.Codex/worker/` | Hourly + per-minute /Codex task scheduler scripts (see `.Codex/README.md`) |
...
## `/Codex` operator queue

See `.Codex/README.md` for the end-to-end install of the `MADAuthorClaudeWorker`
and `MADAuthorClaudeScanner` scheduled tasks. Logs land in
`.Codex/worker/worker.log` and `.Codex/scanner/scanner.log`.
